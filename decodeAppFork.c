...
void eliminar_archivos_txt_bin() {
    DIR *directorio;
    struct dirent *entrada;
    ...
    while ((entrada = readdir(directorio)) != NULL) {
        // Verificar si el archivo tiene la extensión .bin
...
int main(int argc, char *argv[]) {
    ...
    struct dirent *entrada;
    char filePath[MAX_PATH_LENGTH];
    char command[MAX_PATH_LENGTH * 2]; // Tamaño suficiente para el comando
    pid_t pid;
    ...
    while ((entrada = readdir(directorio)) != NULL) {
        ...
        if (strstr(entrada->d_name, ".bin") != NULL && strcmp(entrada->d_name, "comprimido.bin") != 0) {
            ...
            if (pid == 0) {
                // En el proceso hijo
                execlp("./decode", "decode", filePath, (char *)NULL);
